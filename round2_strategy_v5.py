...
IPR_EOD_START_FRAC    = 0.985   # 推迟至98.5%：每推迟1%×10000tick×0.1×80手=+800pts
IPR_EOD_HARD_FRAC     = 0.995   # 安全窗口：98.5~99.5%有100tick >> 8tick需求
...
ACO_STRONG_SIGNAL     = 0.90  # 原值，0.60 降阈效果不明显，ACO 受限于市场机会数
    ...
    def _ipr_orders(self, depth, pos: int, limit: int,
        ...
        if pos > 0 and timestamp >= eod_start_ts:
            ...
            if timestamp >= eod_hard_ts:
                ...
                # 分批清仓：每 tick 最多卖 10 手，避免一次性吃穿 bid1 进入 bid2
                # 全量市价会导致 LOB 冲击（bid1 只有 ~10 手），多付约 3pts/手
                chunk = min(10, pos)
                orders.append(Order("INTARIAN_PEPPER_ROOT", best_bid, -chunk))
            return orders
