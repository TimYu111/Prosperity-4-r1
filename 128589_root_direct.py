...
IPR_OPEN_AGGR_TS = 150_000
IPR_EOD_START_TS = 940_000
IPR_EOD_HARD_TS = 985_000
IPR_CROSS_EDGE_EARLY = 2.0
IPR_CROSS_EDGE_LATE = 4.0
...
ACO_EOD_START_TS = 950_000
ACO_EOD_HARD_TS = 995_000
...
class Trader:
    ...
    def _ipr_orders(self, depth, pos: int, limit: int, timestamp: int) -> list[Order]:
        ...
        if pos > 0 and timestamp >= IPR_EOD_START_TS:
            if best_bid is None:
                return orders
            if timestamp >= IPR_EOD_HARD_TS:
                orders.append(Order("INTARIAN_PEPPER_ROOT", best_bid, -pos))
            else:
                # already late in the day: reduce with meaningful chunks
                to_sell = max(1, (pos + 1) // 2)
                px = best_bid if timestamp >= 970_000 else best_bid + 1
                orders.append(Order("INTARIAN_PEPPER_ROOT", px, -min(pos, to_sell)))
            return orders

        # Core idea: get full as early as possible, then stop trading.
        if pos >= limit:
            return orders
        ...
        # Very early session: cross aggressively. ROOT's drift dominates spread.
        if timestamp <= 20_000:
            take = min(buy_cap, ask_vol if ask_vol > 0 else buy_cap)
            if take > 0:
                orders.append(Order("INTARIAN_PEPPER_ROOT", best_ask, take))
            buy_cap -= take
            if buy_cap > 0 and best_ask + 1 < fv + 6:
                orders.append(Order("INTARIAN_PEPPER_ROOT", best_ask + 1, buy_cap))
            return orders

        # After the opening window, only add if still clearly cheap versus trend.
        edge = fv - best_ask
        if edge >= 3.0:
            take = min(buy_cap, ask_vol if ask_vol > 0 else buy_cap)
            if take > 0:
                orders.append(Order("INTARIAN_PEPPER_ROOT", best_ask, take))
        elif best_bid is not None and timestamp <= 150_000:
            passive_bid = min(best_bid + 1, int(fv - 1))
            if passive_bid > 0:
    ...
    def _aco_orders(self, depth, pos: int, limit: int, timestamp: int) -> list[Order]:
        ...
        if timestamp >= ACO_EOD_START_TS and pos != 0:
            if pos > 0:
                px = best_bid if timestamp >= ACO_EOD_HARD_TS else best_bid + 1
                orders.append(Order("ASH_COATED_OSMIUM", px, -pos))
            else:
                px = best_ask if timestamp >= ACO_EOD_HARD_TS else best_ask - 1
                orders.append(Order("ASH_COATED_OSMIUM", px, -pos))
            return orders
    ...
    def run(self, state: TradingState):
        ...
        # Detect new day by timestamp reset as a fallback.
