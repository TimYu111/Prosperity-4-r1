...
# ============================================================
ACO_FAIR              = 10000
ACO_BASE_SIZE         = 20     # limit=80 → 每次报约20手（vs R1的5手）
ACO_INVENTORY_SOFT    = 40     # 软库存警戒（vs R1的10）
ACO_INVENTORY_HARD    = 64     # 硬库存警戒（vs R1的16），80*0.8
    ...
    def _ipr_orders(self, depth, pos: int, limit: int,
        ...
        if pos > 0 and timestamp >= eod_start_ts:
            ...
            if timestamp >= eod_hard_ts:
                ...
                # 被动挂 FV-4（通常比 bid+1 高，更容易成交）
                target_px = int(round(fv - IPR_EOD_PASSIVE_OFFSET))
