...
DRIFT_R2_THRESH  = 0.15  # R² 阈值降低：更早识别趋势
...
class Trader:
    ...
    def _emerald_orders(self, depth, pos: int, limit: int,
        ...
        if timestamp >= EOD_START_TS:
            return self._eod_unwind("EMERALDS", depth, pos, limit, timestamp)
    ...
    def _tomato_orders(self, depth, pos: int, limit: int,
        ...
        # ── EOD 平仓 ────────────────────────────────────────
        if timestamp >= EOD_START_TS:
            return self._eod_unwind("TOMATOES", depth, pos, limit, timestamp)
        ...
        # OBI 只影响 fv → reservation_price → 被动报价偏向
        # 不在这里主动吃单（成本 6.5 >> OBI 预测收益 1~2，必亏）
        ...
        # 库存警戒：用被动单而非市价，避免踩踏自己
        if pos >= limit * INVENTORY_ALERT:
            reduce = int(pos - limit * 0.5)
            if reduce > 0 and best_bid is not None:
                orders.append(Order("TOMATOES", best_bid + 1, -reduce))
            return orders
        if pos <= -limit * INVENTORY_ALERT:
            reduce = int(-pos - limit * 0.5)
            if reduce > 0 and best_ask is not None:
                orders.append(Order("TOMATOES", best_ask - 1, reduce))
            return orders
    ...
    def _eod_unwind(self, symbol: str, depth, pos: int,
        ...
        # 最后 50 tick（ts > 195000）才用市价，否则用被动单
        # 避免 EOD 时在静态 bot 价格（10008/9992）成交，损失 15pt
        urgent = (timestamp >= 195000)

        if pos > 0 and best_bid is not None:
            price = best_bid if urgent else best_bid + 1
            orders.append(Order(symbol, price, -pos))
