...
class Trader:
    ...
    def _ipr_orders(self, depth, pos: int, limit: int, timestamp: int) -> list[Order]:
        ...
        # Exit in stages; edge is in holding, not squeezing last tick
        if pos > 0 and timestamp >= IPR_EOD_START_TS:
            ...
            if timestamp >= IPR_EOD_HARD_TS:
                ...
                to_sell = max(1, pos // 3)
                price = best_bid if timestamp >= 970_000 else best_bid + 1
                orders.append(Order("INTARIAN_PEPPER_ROOT", price, -min(pos, to_sell)))
            return orders
        ...
        edge = fv - best_ask
        # Early in day, fill faster. Later, be a bit more selective.
        cross_edge = IPR_CROSS_EDGE_EARLY if timestamp <= IPR_OPEN_AGGR_TS else IPR_CROSS_EDGE_LATE
